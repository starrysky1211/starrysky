...
class _ClientAPI(QObject):
    ...
    def _load_repodata(self, filepaths, metadata=None, python_version=None):
        ...
        private_packages = self._multi_packages(
            logins=logins,
            access='private',
        )
        ...
        # Include private packages inside all_packages
        for pkg in private_packages:
            if pkg in all_packages:
                p_data = all_packages.get(pkg)
                versions = p_data.get('versions', '') if p_data else []
                private_versions = list(private_packages[pkg]['versions'])
                all_versions = sort_versions(
                    list(set(list(versions) + private_versions))
                )
                all_packages[pkg] = private_packages[pkg]
                all_packages[pkg]['versions'] = all_versions
            else:
                p_versions = private_packages[pkg]['versions']
                private_versions = sort_versions(p_versions)
    ...
    def logout(self):
        """Logout from anaconda server."""
        logger.debug('Logout')
        method = self._anaconda_client_api.remove_authentication
        return self._create_worker(method)

    def load_repodata(self, filepaths, metadata=None, python_version=None):
        """
        ...
        filepaths: list of str
            Filepaths to download repodata files from (repo.continuum.io) and
            anaconda server (enterprise or cloud).
        metadata: dict
            ...
            for item in data:
                ...
                if name and not public and 'conda' in package_types:
                    if name in private_packages:
                        ...
                        vers = sort_versions(
                            list(set(versions + new_versions))
                        )
