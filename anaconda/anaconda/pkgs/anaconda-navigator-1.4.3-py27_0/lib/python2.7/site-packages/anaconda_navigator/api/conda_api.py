...
class _CondaAPI(QObject):
    ...
    def get_envs(self, log=True):
        ...
        # Temporal fix while conda is used as module
        # non admin installs of admin installed conda is stored on user.
        user_envs_path = os.sep.join([get_home_dir(), '.conda', 'envs'])
        user_envs = []
        if os.path.isdir(user_envs_path):
            user_envs = os.listdir(user_envs_path)
            user_envs = [os.sep.join([user_envs_path, i]) for i in user_envs]

        envs_path = os.sep.join([self.ROOT_PREFIX, 'envs'])
        envs = []
        if os.path.isdir(envs_path):
            envs = os.listdir(envs_path)
            envs = [os.sep.join([envs_path, i]) for i in envs]

        valid_envs = [
            e for e in envs + user_envs
