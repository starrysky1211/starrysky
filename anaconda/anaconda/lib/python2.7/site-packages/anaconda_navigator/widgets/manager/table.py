...
#            self.action_released()
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton and column == C.COL_ACTION:
            if model.is_installable(model_index):
                ...
                self.action_pressed(index)
                self.context_menu_requested(event)
        elif (event.button() == Qt.RightButton and column == C.COL_ACTION and
              index):
            self.action_pressed(index)
            self.context_menu_requested(event)
        elif (event.button() == Qt.LeftButton and column == C.COL_VERSION and
              self.proxy_model is not None and
              model.is_upgradable(model_index)):
            row_data = self.source_model.row(model_index.row())
            name = row_data[C.COL_NAME]
            versions = self.source_model.get_package_versions(name)
    ...
    def action_pressed(self, index):
        ...
        if self.proxy_model is not None:
            ...
            if (column == C.COL_INSTALL and model.is_installable(model_index)):
                model.update_row_icon(model_index.row(), C.COL_INSTALL)

            elif (column == C.COL_INSTALL and model.is_removable(model_index)):
    ...
    def action_released(self):
        ...
        actions = {
            C.COL_INSTALL: C.ACTION_INSTALL,
        ...
        if model_index:
            ...
            if self.valid:
                ...
                if type_ == C.CONDA_PACKAGE:
                    self.sig_conda_action_requested.emit(
                        name, action, version, versions, self._packages_sizes
                    )
