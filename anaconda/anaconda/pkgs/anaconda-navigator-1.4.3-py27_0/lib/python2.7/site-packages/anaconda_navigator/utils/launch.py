...
from anaconda_navigator.config import HOME_PATH, LINUX, MAC
...
def run_app_on_win(prefix, command, package_name, root_prefix):
    ...
    cmd = ('{CONDA_PREFIX}/bin/activate {CONDA_PREFIX}\n'
           '{COMMAND}\n').format(
               CONDA_PREFIX=prefix, COMMAND=command
           )

    suffix = 'bat'
    fpath = create_app_run_script(
        cmd, package_name, prefix, root_prefix, suffix
    )
    CREATE_NO_WINDOW = 0x08000000
    pid = subprocess.Popen(
        fpath, creationflags=CREATE_NO_WINDOW, shell=True
    ).pid
    return pid
...
def run_app_on_unix(prefix, command, package_name, root_prefix):
    ...
    cmd = (
        ...
        CONDA_PREFIX=prefix, COMMAND=command, start=start
    )
    suffix = 'sh'
    fpath = create_app_run_script(
        cmd, package_name, prefix, root_prefix, suffix
    ...
    process = QProcess()
    res = process.startDetached(fpath, [], '')
