...
class CreateDialog(DialogBase):
    ...
    def refresh(self, text=''):
        ...
        if self.environments:
            check = self.check_r.checkState() or self.check_python.checkState()
            if check and is_valid_env_name(text, self.environments):
                self.button_ok.setDisabled(False)
            else:
                self.button_ok.setDisabled(True)
...
class CloneDialog(DialogBase):
    ...
    def refresh(self, text=''):
        ...
        text = self.text_name.text().strip()
        if self.environments:
            self.button_ok.setDisabled(
                not is_valid_env_name(text, self.environments)
            )
...
class RemoveDialog(DialogBase):
    ...
    def __init__(self, parent=None, environment=None):
        ...
        self.label_environment = QLabel(
            'Do you want to remove environment '
...
class CreateConflictDialog(DialogBase):
    """Create new environment dialog if navigator conflicts with deps."""

    def __init__(
        self, parent=None, environments=None, package=None, extra_message=''
    ):
        """Create new environment dialog if navigator conflicts with deps."""
        super(CreateConflictDialog, self).__init__(parent=parent)
        ...
        # Widgets setup
        self.combo_version.addItems(environments)
        # self.combo_version.setCurrentIndex(versions.index(now))
        self.combo_version.setEditable(True)
        self.combo_version.setCompleter(None)
        self.combo_version.setValidator(get_regex_validator())

        # self.text_name.setPlaceholderText("New environment name")
        self.setMinimumWidth(340)
        self.setWindowTitle(
            "Create new environment for '{}'".format(package_name)
        )
        if package_name.lower() not in [e.lower() for e in environments]:
            self.combo_version.setEditText(package_name)

        # Layouts
        vlayout = QVBoxLayout()
        vlayout.addWidget(self.label_info)
        vlayout.addWidget(SpacerVertical())
        vlayout.addWidget(SpacerVertical())

        hlayout = QHBoxLayout()
        hlayout.addWidget(self.label_name)
        hlayout.addWidget(SpacerHorizontal())
        hlayout.addWidget(self.combo_version)
...
def is_valid_env_name(env, envs):
    """
    ...
    return env and env not in envs
...
def test():  # pragma: no cover
    ...
    widget_create_conflict = CreateConflictDialog(
        parent=None,
        environments=['Test'],
        package='orange3',
    )
