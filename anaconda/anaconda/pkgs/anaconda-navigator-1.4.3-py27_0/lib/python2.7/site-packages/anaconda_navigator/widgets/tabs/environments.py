...
class EnvironmentsTab(WidgetBase):
    ...
    def __init__(self, parent=None):
        ...
        self.list_environments.sig_item_selected.connect(self.load_environment)
    ...
    def update_visibility(self, enabled=True):
        ...
        self.sig_status_changed.emit(not enabled)
    ...
    def update_style_sheet(self, style_sheet=None):
        ...
        palette = {
            ...
            QIcon(images.CONDA_MANAGER_NOT_INSTALLED),
            'icon.action.installed': QIcon(images.CONDA_MANAGER_INSTALLED),
            'icon.action.remove': QIcon(images.CONDA_MANAGER_REMOVE),
            'icon.action.add': QIcon(images.CONDA_MANAGER_ADD),
            'icon.action.upgrade': QIcon(images.CONDA_MANAGER_UPGRADE),
            'icon.action.downgrade': QIcon(images.CONDA_MANAGER_DOWNGRADE),
            'icon.upgrade.arrow': QIcon(images.CONDA_MANAGER_UPGRADE_ARROW),
        ...
        self.packages_widget.update_style_sheet(
            ...
            extra_dialogs={
                'cancel_dialog': ClosePackageManagerDialog,
    ...
    def ordered_widgets(self, next_widget=None):
        ...
        if next_widget is not None:
            self.packages_widget.table_last_row.add_focus_widget(next_widget)
        ordered_widgets = [self.text_search, ]
        ordered_widgets += self.list_environments.ordered_widgets()
        ordered_widgets += [
            self.button_create,
            self.button_clone,
            self.button_import,
            ...
            self.packages_widget.table,
            self.packages_widget.table_last_row,
    ...
    def setup_tab(
        ...
        def select(value=None, position=None):
            ...
            self.menu_list.actions(
            )[2].setEnabled(launch.check_prog('ipython', prefix))
            self.menu_list.actions(
            )[3].setEnabled(launch.check_prog('notebook', prefix))
    ...
    def load_environment(self, item=None):
        ...
        index = [
            i for i, it in enumerate(self.list_environments._items)
    ...
    def _environment_created(self, worker, output, error):
        ...
        if is_error or error or not success:
            ...
            alert = MessageBoxError(
                text="Environment Create Error", error=error
            )
    ...
    def _environment_removed(self, worker, output, error):
        ...
        if error:
            ...
            if "error" in error:
                alert = MessageBoxError(
                    text="Environment Removal Error", error=error
                )
                self.sig_error_popped_up.emit()
                alert.exec_()
        self.sig_environments_updated.emit()
    ...
    def create_environment(self):
        ...
        self.tracker.track_page(
            '/environments/create', pagetitle='Create new environment dialog'
        )
    ...
    def remove_environment(self):
        ...
        if current_item is not None:
            ...
            self.tracker.track_page(
                '/environments/remove', pagetitle='Remove environment dialog'
            )
            if dlg.exec_():
