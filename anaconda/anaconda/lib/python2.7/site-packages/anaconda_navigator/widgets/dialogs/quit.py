...
class QuitRunningAppsDialog(DialogBase):
    ...
    def accept(self):
        ...
        Update the configuration preferences.
        """
        original_checked_packages = self.config.get(
            'main', 'running_apps_to_close'
        )
        hide_dialog = self.checkbox.checkState() == Qt.Checked
        checked_packages = [i.text() for i in self.list.items() if i.checked()]
        packages = list(set(checked_packages + original_checked_packages))
        self.config.set('main', 'running_apps_to_close', packages)
